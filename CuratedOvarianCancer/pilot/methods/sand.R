...
make_proxy_preds_linear <- function(target, proxy_list) {
  K  <- length(proxy_list)
  Xt <- as.matrix(target$X)          # n_t × p1
  nt <- nrow(Xt)

  lapply(seq_len(K), \(k) {
    prxk <- proxy_list[[k]]
    Xp   <- as.matrix(prxk$D$X)
    Zp   <- as.matrix(prxk$D$Z)
    Yp   <- as.numeric(prxk$y)

    # (a) Linear feature map estimation: hat(P)_p via column-wise Lasso
    P_hat <- cv.fmap(Xp, Zp, 'linear')

    # (b) Proxy regression: fit ridge on full proxy design [X_p, Z_p]
    Dp    <- cbind(Xp, Zp)
    Proxy_fit <- cv.glmnet(Dp, Yp, alpha = 1) # lasso on proxy
    omega_hat <- coef(Proxy_fit, s = 'lambda.min') # intercept + p coefs
    ...
    as.numeric(cbind(1, Xt) %*% (omega1 + P_hat %*% omega2))
  })
}


#' Signal-Based Adaptive Negative Transfer Defense (SAND)
run_sand <- function(Xt, yt, proxy_preds_list, gamma_scale = 1) {
  Xt <- as.matrix(Xt)
  yt <- as.numeric(yt)
  nt  <- nrow(Xt)
  p1  <- ncol(Xt)
  ...
  # ==================================================================
  cv_base  <- cv.glmnet(Xt, yt, alpha = 1)
  idx_1se <- which.min(abs(cv_base$lambda - cv_base$lambda.1se))
  S_base  <- cv_base$cvm[idx_1se]
  
  idx_min <- which(cv_base$lambda == cv_base$lambda.1se)
  s_hat   <- cv_base$nzero[idx_min]
  y_hat   <- predict(cv_base, newx = Xt, s = "lambda.1se")
  rss     <- sum((yt - drop(y_hat))^2)
...
run_sand_pipeline <- function(D_data, sp = "nsp", gamma_n = NULL) {
  ...
  hA <- run_sand(
    Xt = target$X, yt = target$y, proxy_preds_list = proxy_preds, gamma_n = gamma_n
  )
  hA
}