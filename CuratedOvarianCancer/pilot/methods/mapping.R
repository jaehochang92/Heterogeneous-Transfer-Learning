...
cv.fmap <- function(X, Z, h = "sieve", n_folds = 5) {
  p1 <- ncol(X)
  p2 <- ncol(Z)
  n <- nrow(X)
  ...
  if (!is.na(slurm_cores) && slurm_cores > 0) {
    ...
    n_cores <- max(1L, detected_cores - 2L)
  }
  if (h == "linear") {
    ...
    ϴ <- lapply(fit.list, function(fit)
      coef(fit, s = 'lambda.1se'))  # keep intercept row
    return(do.call(cbind, ϴ))
  }
  # Sieve
  sv <- pbmcapply::pbmclapply(
    1:p2, function(j) .cv.sieve(X, Z[, j]), mc.cores = n_cores
    )
  sv <- pbmcapply::pbmclapply(1:p2, function(j) {
    ...
    sv_fit$beta_hat  # keep full beta_hat: Phi[,1]=1 carries the intercept
  }, mc.cores = n_cores)
  cbind_padded_vecs(sv)
}

# Predict missing features using a pretrained feature map (fmap)
cv.fmap.predict <- function(Θ, X, model = "sieve", base_min = NULL, 
                         base_max = NULL, sieve_type = "cosine") {
  if (model == "linear") {
    return(cbind(1, X) %*% Θ)
  }
  if (!exists("sieve_preprocess", mode = "function")) {
    stop("sieve_preprocess() is required for model = 'sieve'.")
  }
  if (is.null(base_min)) base_min <- apply(X, 2, min)
  if (is.null(base_max)) base_max <- apply(X, 2, max)
  X_decoy <- rbind(base_min, base_max, X)
  Phi_decoy <- sieve_preprocess(X_decoy, nrow(Θ), type = sieve_type)$Phi
  Phi <- Phi_decoy[-(1:2), , drop = FALSE]
  Phi %*% Θ
}