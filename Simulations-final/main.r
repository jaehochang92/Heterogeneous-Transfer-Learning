...
if (!dir.exists(log_dir))
  dir.create(log_dir)
log_path <- sprintf("%s/progress_task_%04d.log", log_dir, task_id)
...
task_cfg <- sweep_plan[task_id]
sp_cases <- c("sp", "nsp")
total_steps <- opt$rep * length(sp_cases)
log_message(
  sprintf(
    "Start task=%d/%d sweep_param=%s K=%d np=%d nt=%d p1=%d p2=%d rep=%d",
    task_id,
    total_tasks,
...
for (i in seq_len(opt$rep)) {
  ...
  for (sp_case in sp_cases) {
    for (fmap in c('linear', 'nlinear')) {
      t0 <- proc.time()[3]
      log_message(
        sprintf(
          "Progress %d/%d: iter=%d sp_case=%s dgp_fmap=%s seed=%d",
          row_idx, total_steps, i, fmap, sp_case, current_seed
        )
      )
      ran_once <- sim.runner(
        seedno = current_seed,
      ...
      if (row_idx %% 2L == 0L || row_idx == total_steps) {
        ...
        partial_path <- sprintf("results/sim_res_task_%04d.partial.rds", task_id)
...
save_path <- sprintf("%s/sim_res_task_%04d.rds", out_dir, task_id)
saveRDS(final_dt, save_path)
log_message(sprintf(
  "Completed task=%d rows=%d saved=%s",
  task_id,
  nrow(final_dt),
  save_path
))
cat(sprintf("Task %d completed. Saved to %s\n", task_id, save_path))